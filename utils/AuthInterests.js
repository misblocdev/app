...
const setFavoriteHos = async data => {
  let rsData = { code: 0, msg: '', other: '' };

  try {
    const USER_ENDPOINT = `${SERVER_URL}/interests/setFavoriteHos`;
...
const getMedicalSubject = async data => {
  let rsData = { code: 0, msg: '', other: '' };

  try {
    const USER_ENDPOINT = `${SERVER_URL}/interests/cate`;
    let oauth_access_token_id = await getAuthStore('access_token');
    const sHeaders = secureCmmConfig;
    sHeaders.headers.Authorization = 'Bearer ' + oauth_access_token_id;
    await axios
...
const getPostArea = async data => {
  let rsData = { code: 0, msg: '', other: '' };

  try {
    const USER_ENDPOINT = `${SERVER_URL}/interests/area`;
    let oauth_access_token_id = await getAuthStore('access_token');
    const sHeaders = secureCmmConfig;
    sHeaders.headers.Authorization = 'Bearer ' + oauth_access_token_id;
    await axios
...
const getMyFavorite = async data => {
  let rsData = { code: 0, msg: '', other: '' };

  try {
    const USER_ENDPOINT = `${SERVER_URL}/interests`;
    let oauth_access_token_id = await getAuthStore('access_token');
    const sHeaders = secureCmmConfig;
    sHeaders.headers.Authorization = 'Bearer ' + oauth_access_token_id;
    await axios
...
const setMyFavoriteReg = async data => {
  let rsData = { code: 0, msg: '', other: '' };

  try {
    const USER_ENDPOINT = `${SERVER_URL}/interests/reg`;
...
const delMyFavorite = async data => {
  let rsData = { code: 0, msg: '', other: '' };

  try {
    const USER_ENDPOINT = `${SERVER_URL}/interests/del`;
